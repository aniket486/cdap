 ...
 *
 * PollingDelete -- (state == NOT_FOUND && timeout reached) --> Failed
 */
public class ProvisionTask extends ProvisioningTask {
  ...
  protected Map<ProvisioningOp.Status, ProvisioningSubtask> getSubtasks() {
    ImmutableMap.Builder<ProvisioningOp.Status, ProvisioningSubtask> subtasks = ImmutableMap.builder();

    // RequestingCreate
    ProvisioningSubtask subtask = new ClusterCreateSubtask(provisioner, provisionerContext, cluster -> {
      if (cluster == null) {
    ...
    subtasks.put(ProvisioningOp.Status.REQUESTING_CREATE, subtask);

    // PollingCreate
    subtask = new ClusterPollSubtask(provisioner, provisionerContext, ClusterStatus.CREATING, cluster -> {
    ...
    subtasks.put(ProvisioningOp.Status.POLLING_CREATE, subtask);

    // RequestingDelete
    subtask = new ClusterDeleteSubtask(provisioner, provisionerContext,
    ...
    subtask = new ClusterInitializeSubtask(provisioner, provisionerContext, cluster -> {
      provisionerNotifier.provisioned(programRunId, initialTaskInfo.getProgramOptions(),
                                      ...
                                      cluster, initialTaskInfo.getSshKeyInfo());
      return Optional.of(ProvisioningOp.Status.CREATED);
    });
    subtasks.put(ProvisioningOp.Status.INITIALIZING, subtask);

    // states for Failed and Created
